...
const MOCK_CATEGORIES = [
  { id: 1, code: 'TDCS', name: 'Các số tiêu điểm chính sách trên VTV1', slug: 'chuong-trinh-tieu-diem-chinh-sach-tren-vtv-1', sitemap: 48 },
  { id: 2, code: 'NQ66', name: 'Tin Ban Chỉ đạo Nghị quyết số 66-NQ/TW', slug: 'tin-ban-chi-dao-nghi-quyet-so-66-nq-tw', sitemap: 5 },
  { id: 3, code: 'NPLVN', name: 'Ngày Pháp luật Việt Nam', slug: 'ngay-phap-luat-viet-nam', sitemap: 53 },
  { id: 4, code: 'CCHD', name: 'Cắt giảm, đơn giản hóa thủ tục hành chính d...', slug: 'cat-giam-don-gian-hoa-thu-tuc-hanh-chinh-dua-tren...', sitemap: 13 },
  { id: 5, code: 'TCQT', name: 'Trung tâm tài chính quốc tế tại Việt Nam', slug: 'trung-tam-tai-chinh-quoc-te-tai-viet-nam', sitemap: 11 },
  { id: 6, code: 'KHT10', name: 'Kỳ họp thứ 10, Quốc hội khóa XV', slug: 'ky-hop-thu-10-quoc-hoi-khoa-xv', sitemap: 9 },
  { id: 7, code: 'TVNQ', name: 'Toàn văn Nghị quyết', slug: 'toan-van-nghi-quyet', sitemap: 54 },
  { id: 8, code: 'INFOR', name: 'Infographic', slug: 'infographic1', sitemap: 50 },
  { id: 9, code: 'THD', name: 'Tin hoạt động', slug: 'tin-hoat-dong', sitemap: 17 },
  { id: 10, code: 'TSPL', name: 'Thời sự pháp luật', slug: 'thoi-su-phap-luat', sitemap: 15 },
  { id: 11, code: 'CSM', name: 'Chính sách mới', slug: 'chinh-sach-moi', sitemap: 19 },
  { id: 12, code: 'DHXIV', name: 'Đại hội lần thứ XIV của Đảng', slug: 'dai-hoi-lan-thu-xiv-cua-dang', sitemap: 45 },
  { id: 13, code: 'LAN', name: 'Legal Aid News & Activities', slug: '--', sitemap: 105 },
  { id: 14, code: 'RS66', name: 'Resolution 66', slug: '--', sitemap: 64 },
  { id: 15, code: 'multimedia', name: 'Multimedia', slug: '--', sitemap: 70 },
  { id: 16, code: 'CTTC', name: 'Câu chuyện thành công', slug: '--', sitemap: 67 }
];

export default function CategoriesManagementPage() {
  const [columns, setColumns] = useState([
    { id: 'code', label: 'Mã', visible: true, width: 150, align: 'left' },
    { id: 'name', label: 'Tên danh mục', visible: true, width: 400, align: 'left' },
    { id: 'slug', label: 'Slug tự sinh (URL) để gắn vào Sitemap', visible: true, width: 450, align: 'left' },
  ...
  const toggleSelectAll = () => {
    ...
    else setSelectedIds(MOCK_CATEGORIES.map(n => n.id));
  };
  ...
  const sortedData = [...MOCK_CATEGORIES].sort((a, b) => {
  ...
  const isAllSelected = selectedIds.length === MOCK_CATEGORIES.length && MOCK_CATEGORIES.length > 0;
  const isIndeterminate = selectedIds.length > 0 && selectedIds.length < MOCK_CATEGORIES.length;
      ...
      <header className="h-[72px] border-b border-gray-200 flex items-center justify-between px-6 shrink-0 bg-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-xl font-bold text-[#14233b]">Quản lý danh mục</h1>
          </div>
        </div>
        
        <div className="flex items-center gap-4">
          <span className="text-sm font-medium text-gray-400">Có {MOCK_CATEGORIES.length} bản ghi</span>
          ...
          {selectedIds.length > 0 ? (
             <div className="flex items-center gap-2">
               ...
               <Link href="/content/categories/create" className="w-10 h-10 rounded-full bg-[#5340FF] text-white hover:bg-[#4330EF] shadow-md shadow-[#5340FF]/20 flex items-center justify-center transition-colors ml-2">
                  ...
                  {columns.map(col => {
                    ...
                    return (
                      <div key={col.id} className={cn("px-4 flex items-center", col.align === 'center' ? 'justify-center' : col.align === 'right' ? 'justify-end' : 'justify-start')} style={{ width: col.width, minWidth: col.width }}>
                        <span className={cn("truncate text-[13px] font-medium w-full", col.id === 'code' ? 'text-gray-400' : 'text-gray-700')} style={{ textAlign: col.align as any }}>
